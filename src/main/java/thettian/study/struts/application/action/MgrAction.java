...
public class MgrAction extends BaseAction {
	...
	@Action(value = "index", results = { @Result(name = SUCCESS, location = "/manager/index.jsp") })
	public String index() {
		return SUCCESS;
	}

	@Action(value = "newpass", results = { @Result(name = SUCCESS, location = "/manager/newpass.jsp") })
	public String newpass() {
		return SUCCESS;
	}

	@Action(value = "checktask", results = { @Result(name = SUCCESS, location = "/manager/checktask.jsp") })
	public String checkTask() {
		return SUCCESS;
	}

	@Action(value = "updatetask", results = {
			@Result(name = SUCCESS, location = "/manager/updatetask.jsp"),
			@Result(name = "finish", location = "/manager/finishtask.jsp") })
	...
	@Action(value = "updateitem", results = { @Result(name = SUCCESS, location = "/manager/checktask.jsp") })
	public String updateItem() {
		...
		System.out.println(_u.getUsername());
		_task.setExeuser(_u);
		if (task.getStatus().equals("已完成")) {
	...
	public List<Task> getTasks() {
		User u = (User) session.get("user");
		List<Task> ls = managerService.findDeptTasks(u.getDepartment().getId());
		if (ls.size() > 0)
			return ls;
		else
			return null;
	...
	public List<User> getWorkers() {
		User u = (User) session.get("user");
		List<User> ls = managerService
				.findUserByDept(u.getDepartment().getId());
		if (ls.size() > 0)
			return ls;
		else
			return null;
