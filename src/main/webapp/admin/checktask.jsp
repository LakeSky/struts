			...
			<div class="span9" style="padding-left: 20px;">
				...
				<div id="demo">
					<form id="checkperson" class="form-horizontal"
						action="${ baseUrl }/admin/deletetask" method="post">
							...
							<tbody>
								<c:if test="${tasks != null }">
									<c:forEach var="task" items="${ tasks }" varStatus="stat">
										...
										<td class="center"><a
											href="${ baseUrl }/admin/updatetask?Taskid=${task.id }">${task.name
												}</a></td>
										<td class="center">${task.exedep.name }</td>
										<td class="center">${task.exeuser.username }</td>
										<td class="center"><input type="checkbox" name="taskids"
											value="${task.id }"></td>
										</tr>
									</c:forEach>
								</c:if>
